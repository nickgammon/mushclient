...
static int lp_locale (lua_State *L) {
  // added these lines about the locale otherwise the re test fails
  // not sure why, maybe the Lua DLL and the main code
  // have different locales.
  char old_locale[256];
  strcpy(old_locale, setlocale(LC_CTYPE, NULL)); /* store the locale */
  setlocale (LC_CTYPE, "C");   // fudge by Nick Gammon
